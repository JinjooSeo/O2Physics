...
struct TaskXicXiPiPi {
  ...
  Configurable<std::vector<double>> bins{"pTBins", std::vector<double>{hf_cuts_xic_toxipipi::pTBins_v}, "pT bin limits"};
  ...
  Partition<soa::Join<aod::HfCandCascProng3, aod::HFSelXicCandidate>> selectedXicCandidates = aod::hf_selcandidate_xic::isSelXicToXiPiPi >= d_selectionFlagXic || aod::hf_selcandidate_xic::isSelXicToPiPiXi >= d_selectionFlagXic;

  void process(soa::Join<aod::HfCandCascProng3, aod::HFSelXicCandidate> const& candidates)
  {
    for (auto& candidate : selectedXicCandidates) {
      ...
      if (candidate.isSelXicToXiPiPi() >= d_selectionFlagXic) {
        registry.fill(HIST("hMass"), InvMassXic(candidate), candidate.pt());
      }
      if (candidate.isSelXicToPiPiXi() >= d_selectionFlagXic) {
        registry.fill(HIST("hMass"), InvMassXicbar(candidate), candidate.pt());
      }
      ...
      registry.fill(HIST("hSelectionStatus"), candidate.isSelXicToXiPiPi(), candidate.pt());
      registry.fill(HIST("hSelectionStatus"), candidate.isSelXicToPiPiXi(), candidate.pt());
  ...
  Partition<soa::Join<aod::HfCandCascProng3, aod::HFSelXicCandidate, aod::HfCandCascProng3MCRec>> selectedMCXicCandidates = (aod::hf_selcandidate_xic::isSelXicToXiPiPi >= d_selectionFlagXic || aod::hf_selcandidate_xic::isSelXicToPiPiXi >= d_selectionFlagXic);

  void processMC(soa::Join<aod::HfCandCascProng3, aod::HFSelXicCandidate, aod::HfCandCascProng3MCRec> const& candidates,
                 soa::Join<aod::McParticles, aod::HfCandCascProng3MCGen> const& particlesMC, aod::BigTracksMC const&)
  {
    // MC rec.
    for (auto& candidate : selectedMCXicCandidates) {
      ...
      if (candidate.isSelXicToXiPiPi() >= d_selectionFlagXic) {
        mass = InvMassXic(candidate);
      }
      if (candidate.isSelXicToPiPiXi() >= d_selectionFlagXic) {
        massC = InvMassXicbar(candidate);
      }

      if (std::abs(candidate.flagMCMatchRec()) == 1 << DecayType::XicToXiPiPi) {
        // Signal
        auto indexMother = RecoDecay::getMother(particlesMC, candidate.index0_as<aod::BigTracksMC>().mcParticle_as<soa::Join<aod::McParticles, aod::HfCandCascProng3MCGen>>(), pdg::Code::kXiCPlus, true);
    ...
    for (auto& particle : particlesMC) {
      if (std::abs(particle.flagMCMatchGen()) == 1 << DecayType::XicToXiPiPi) {
  ...
  PROCESS_SWITCH(TaskXicXiPiPi, processMC, "Process MC", false);
};

WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  return WorkflowSpec{adaptAnalysisTask<TaskXicXiPiPi>(cfgc, TaskName{"hf-task-xic-toxipipi"})};
}
